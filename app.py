...
def crear_reserva():
    data = request.get_json()
    resp = supabase.table("reservas").insert({
...
def listar_reservas():
    resp = supabase.table("reservas").select("*").execute()
    return jsonify(resp.data), 200

@app.route("/api/reservas/miembro/<id_miembro>", methods=["GET"])
def reservas_por_miembro(id_miembro):
    resp = supabase.table("reservas").select("*").eq("id_miembro", id_miembro).execute()
    return jsonify(resp.data), 200

@app.route("/api/reservas/<id_reserva>", methods=["PUT"])
def actualizar_reserva(id_reserva):
    data = request.get_json()
    resp = supabase.table("reservas").update(data).eq("id_reserva", id_reserva).execute()
    return jsonify(resp.data), 200

@app.route("/api/reservas/<id_reserva>", methods=["DELETE"])
def eliminar_reserva(id_reserva):
    resp = supabase.table("reservas").delete().eq("id_reserva", id_reserva).execute()
    return jsonify(resp.data), 200

# FUNCIONES SECCION NOTIFICACIONES MODULO CLASES - ENTRENADOR

@app.route("/api/notificaciones", methods=["POST"])
def crear_notificacion_miembro():
    data = request.get_json()
    tipo = data.get("tipo_notificacion", "clase")
    if tipo not in ["clase", "promocion", "recordatorio"]:
        tipo = "clase"
    icono = data.get("imagen_url", "/static/uploads/icon.png")
    fecha = data.get("fecha") or f"{str(datetime.now().year)}-{str(datetime.now().month).zfill(2)}-{str(datetime.now().day).zfill(2)}"
    hora = data.get("hora")
    insert_data = {
        "id_miembro": data["id_miembro"],
        "titulo": data["titulo"],
    ...
    resp = supabase.table("notificaciones_miembros").insert(insert_data).execute()
    return jsonify(resp.data), 200

@app.route("/api/notificaciones/miembro/<id_miembro>", methods=["GET"])
def notificaciones_por_miembro(id_miembro):
    resp = supabase.table("notificaciones_miembros").select("*").eq("id_miembro", id_miembro).execute()
    return jsonify(resp.data), 200

@app.route("/api/notificaciones/<id_notificacion>", methods=["GET"])
def notificacion_id(id_notificacion):
    resp = supabase.table("notificaciones_miembros").select("*").eq("id_notificacion_miembro", id_notificacion).single().execute()
    return jsonify(resp.data), 200

@app.route("/api/notificaciones/<id_notificacion>", methods=["PUT"])
def actualizar_notificacion_miembro(id_notificacion):
    data = request.get_json()
    tipo = data.get("tipo_notificacion", "clase")
    if tipo not in ["clase", "promocion", "recordatorio"]:
        tipo = "clase"
    ...
    resp = supabase.table("notificaciones_miembros").update(update_data).eq("id_notificacion_miembro", id_notificacion).execute()
    return jsonify(resp.data), 200

@app.route("/api/notificaciones/<id_notificacion>", methods=["DELETE"])
def eliminar_notificacion_miembro(id_notificacion):
    resp = supabase.table("notificaciones_miembros").delete().eq("id_notificacion_miembro", id_notificacion).execute()
    return jsonify(resp.data), 200
...
# FUNCIONES SECCION CREAR|EDITAR USUARIO MODULO GESTION USUSARIOS - ADMINISTRADOR
...
# FUNCIONES SECCION CREAR|EDITAR USUARIO MODULO ANALISIS|REPORTES - ADMINISTRADOR
...
# FUNCIONES SECCION SISTEMA PUBLICIDAD MODULO SISTEMA PUBLICIDAD - ADMINISTRADOR
